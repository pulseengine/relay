//! Compose the five published stage components into the cascade socket,
//! producing a SELF-CONTAINED component a host can instantiate.
//!
//! Replaces scripts/compose-cascade.sh. The shell version had a real bug on its
//! first run, and it was a shell bug rather than a logic one:
//!
...
//! defined type`. Selecting files by an explicit predicate instead of a glob
//! makes that class unrepresentable.
//!
//! Usage:
//!   compose-cascade <dir-with-published-components> [out.wasm]
...
/// `falcon-<stage>-v<major>.<minor>.wasm` and nothing else.
///
/// Anchored on `-v` followed by a digit precisely because `falcon-cascade-` is a
/// PREFIX of `falcon-cascade-stream-composed-`. Returning an error on anything
/// other than exactly one match means an ambiguous bundle is a loud failure
/// rather than a silent wrong pick.
fn find_one(dir: &Path, stage: &str) -> Result<PathBuf> {
    ...
    match hits.len() {
        ...
        n => bail!("expected exactly 1 falcon-{stage}-v<N>.wasm in {}, found {n}: {hits:?}",
                   dir.display()),
    }
}

/// Component vs core module, read from the payload rather than from a filename
/// or an OCI mediaType. `falcon-rate:1.129.0` shipped as a raw core module past
/// a green check because the manifest said "component" and nobody looked at the
/// bytes.
fn assert_component(path: &Path) -> Result<()> {
    let bytes = std::fs::read(path).with_context(|| format!("reading {}", path.display()))?;
    let head = bytes.get(..8).unwrap_or_default();
    match head {
        [0x00, 0x61, 0x73, 0x6d, 0x0d, 0x00, 0x01, 0x00] => Ok(()),
        [0x00, 0x61, 0x73, 0x6d, 0x01, 0x00, 0x00, 0x00] => {
            bail!("{} is a CORE MODULE, not a component", path.display())
        }
        _ => bail!("{} is not wasm (first 8 bytes: {head:02x?})", path.display()),
    }
}

/// Count `memory.grow` in every code section. The composed cascade must have
/// zero: a growing linear memory cannot be lowered to bare metal, which is the
/// whole point of the no_std conversion (SWREQ-FALCON-OCI-P02).
...
fn main() -> Result<()> {
    ...
    let dir = PathBuf::from(
        args.next()
            .context("usage: compose-cascade <dir-with-published-components> [out.wasm]")?,
    );
    let out = args
        .next()
        .map(PathBuf::from)
    ...
    // plug nobody checked, would otherwise be discovered by a consumer.
