...
fn main() -> Result<()> {
    ...
    // DIFFERENTIAL=1 runs the native mirror alongside and reports the worst
    // per-motor disagreement. This is the "develop in wasm, deploy that wasm"
    // claim made falsifiable: identical Rust by two routes must agree, and if
    // it does not, the Component Model is not transparent and the whole
    ...
    for tick in 0..ticks {
        ...
        // ── DIFFERENTIAL: the same code, reached natively ──────────────────
    ...
    if let Some((_, worst, worst_tick)) = differential.as_ref() {
        ...
        // the same inputs; wasm32 and the host agree on IEEE-754 semantics, so
        ...
        // A tolerance here would hide exactly the divergence worth finding.
        if *worst != 0.0 {
            bail!(
                "FAIL: wasm and native disagree by {worst:.9} (tick {worst_tick}). \
                 The same Rust reached two ways must compute the same answer — if it \
                 does not, 'develop in wasm, deploy that wasm unchanged' is not true \
                 and the difference must be explained before it is tolerated."
            );
        }
