...
n=$(/tmp/plotenv/bin/python -c "import json;print(len(json.load(open('$DATA'))['releases']))")
for k in $(seq 0 $((n-1))); do
  read -r VER < <(/tmp/plotenv/bin/python -c "import json;print(json.load(open('$DATA'))['releases'][$k]['version'])")
  NAR=$(/tmp/plotenv/bin/python -c "import json;print(json.load(open('$DATA'))['releases'][$k]['narration'])")
  TIT=$(/tmp/plotenv/bin/python -c "import json;print(json.load(open('$DATA'))['releases'][$k]['title'])")
  VID="$VIDDIR/falcon-${VER}.mp4"
  [ -f "$VID" ] || { echo "skip $VER (no video $VID)"; continue; }
  echo "== $VER : TTS + mux =="
  # 1) TTS the narration (OpenAI-compatible /v1/audio/speech)
  /tmp/plotenv/bin/python - "$URL" "$MODEL" "$VOICE" "$NAR" /tmp/nar.mp3 <<'PY'
...
PY
  ...
  # 3) channel metadata
  DESC=$(/tmp/plotenv/bin/python -c "import json;print(json.load(open('$DATA'))['releases'][$k]['description'])")
