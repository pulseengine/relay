...
pub fn decide(
    ...
    ensures
        !event_fired ==> result === PersistenceDecision::Pass,
        event_fired && new_count(current_count) >= persistence ==> result === PersistenceDecision::Fire,
        event_fired && new_count(current_count) <  persistence ==> result === PersistenceDecision::Pending,
{
    if !event_fired {
...
pub fn saturating_increment(n: u32) -> (result: u32)
    ensures
        n <  u32::MAX ==> result == n + 1,
        n == u32::MAX ==> result == u32::MAX,
{
    if n < u32::MAX { n + 1 } else { u32::MAX }
}

/// Specification function: what the counter should become after an event.
pub open spec fn new_count(current: u32) -> u32 {
    if current < u32::MAX { (current + 1) as u32 } else { u32::MAX }
}
...
/// whether to store it.
pub fn apply(decision: PersistenceDecision, current_count: u32) -> (result: u32)
    ensures
        decision === PersistenceDecision::Pass ==> result == 0,
        decision === PersistenceDecision::Pending ==> result == new_count(current_count),
        decision === PersistenceDecision::Fire ==> result == new_count(current_count),
{
    match decision {
        PersistenceDecision::Pass => 0,
        PersistenceDecision::Pending | PersistenceDecision::Fire => saturating_increment(current_count),
    }
}
