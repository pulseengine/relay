...
mod tests {
    ...
    #[test]
    fn test_empty() {
        assert_eq!(ScheduleTable::new().process_tick(0, 0).action_count, 0);
    }
    #[test]
    fn test_match() {
        let mut t = ScheduleTable::new();
        t.add_slot(ScheduleSlot {
            minor_frame: 5,
            major_frame: 0,
            target_channel: 42,
            payload_offset: 0,
        ...
        assert_eq!(t.process_tick(5, 1).action_count, 1);
        assert_eq!(t.process_tick(6, 1).action_count, 0);
    }
    #[test]
    fn test_disabled() {
        let mut t = ScheduleTable::new();
        ...
        assert_eq!(t.process_tick(0, 0).action_count, 0);
    }
    #[test]
    fn test_major() {
        let mut t = ScheduleTable::new();
        ...
        assert_eq!(t.process_tick(0, 3).action_count, 1);
        assert_eq!(t.process_tick(0, 2).action_count, 0);
    }
    #[test]
    fn test_bounded() {
        let mut t = ScheduleTable::new();
        ...
        assert_eq!(
            t.process_tick(0, 0).action_count,
            MAX_ACTIONS_PER_TICK as u32
        );
    }
    #[test]
    fn test_full() {
        ...
        for _ in 0..MAX_SCHEDULE_SLOTS {
            assert!(t.add_slot(ScheduleSlot::empty()));
        }
        assert!(!t.add_slot(ScheduleSlot::empty()));
    }
    #[test]
    fn test_enable() {
        ...
        assert_eq!(t.process_tick(0, 0).action_count, 1);
        t.set_enabled(0, false);
        assert_eq!(t.process_tick(0, 0).action_count, 0);
        assert!(!t.set_enabled(99, true));
    }
}
