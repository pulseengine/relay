...
> **HISTORICAL — do not plan from this document's capability claims (#422, 2026-09-17).**
> This is the v1.56 assessment, kept for its narrative. It is 82+ versions stale, and
> its table credits as *shipped* five crates that **nothing we release depends on**:
> `falcon-baromag`, `relay-rc`, `relay-mix-multi`, `relay-sensvote` and
> `relay-modextra` (the last four named in #422; the fifth found by the mechanical
> check). What is actually delivered is derived from the dependency graph in
> **[CAPABILITY-REACHABILITY.md](CAPABILITY-REACHABILITY.md)** — generated by
> `scripts/capability-reachability.rs` and checked current in CI — not from prose.
> Wiring, parking or retiring the unconsumed flight capabilities is
> SWREQ-FALCON-ORPHAN-P01 (v1.141), which is also where a current parity assessment
> belongs.
...
| PX4 capability | Our crate (claimed shipped at v1.56 — see banner) | Release |
|---|---|---|
